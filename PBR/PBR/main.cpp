...
point_light red{ {0,0,3},1.0f,0.022f,0.0019f,0.1f,{1, 1, 1},{1.0f, 1.0f, 1.0f},32 };
...
int main()
{
    ...
    GLuint cube_program = createShader("cv.glsl", "cf.glsl","cg.glsl");
    glm::mat4 model = glm::scale(glm::mat4(1.f), glm::vec3(2.f));
    sendMatrix4(cube_program, model, "model");
    ...
    sendInt(cube_program, 1, "normal");
    GLuint color, metolic, normal, roughness;
    loadTexture(&color, "normal.png");
    //loadTexture(&metolic, "metolic.png");
    loadTexture(&normal, "baseColor.png"); 
    //loadTexture(&roughness, "roughnesss.png");
    float theta = 0;

    while (glfwWindowShouldClose(window) == false)
    {
        glm::mat4 view = glm::lookAt(main_camera.pos, main_camera.pos + main_camera.dir, main_camera.up);
        cout << "camera direction = (" << main_camera.dir.x << "," << main_camera.dir.y << "," << main_camera.dir.z << ")"<<endl;
        glEnable(GL_DEPTH_TEST);
        glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
        glm::vec3 lightPos = glm::vec3(5*sin(glm::radians(1.0*theta)),0,5*cos(glm::radians(1.0*theta)));
        sendVec3(cube_program, lightPos, "red.pos");
